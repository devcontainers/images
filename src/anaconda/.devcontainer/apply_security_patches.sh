...
vulnerable_packages=("pyopenssl=25.0.0" "mistune=3.0.1" "transformers=4.36.0" "cryptography=43.0.1" "jupyter-lsp=2.2.2" "scrapy=2.11.2" \ 
...
for ((i=0; i<rows; i++)); do
    ...
    if [[ $CURRENT_VERSION != $GREATER_VERSION_A ]]; then
        ...
        if [[ $CONDA_VERSION == $GREATER_VERSION_B ]]; then
            ...
            conda install -y "${packages_array[$i,0]}==${CONDA_VERSION}" -c "$channel_name"
            ...
            yes | python3 -m pip install --upgrade --no-cache-dir  "${packages_array[$i,0]}==${REQUIRED_VERSION}"
        fi
    fi
done
